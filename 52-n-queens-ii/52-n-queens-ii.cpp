...
This question is just slightest modification of N Queens problem
There we have to give solution boards in answer, here we just have to just give count of solutions in answer
So change will be just in base case


Approach 1  - Brute force to check Safe condition.
...
public:
    ...
    void check(int n,vector<string>&board,int &count,int col)
    {
        if(col==n)
        {
            count++;
...
Aprroach 2  - Hashing to check Safe condition.
...
{
public:
     void check(int n,vector<string>&board,int &count,int  col,vector<bool>&LeftRow,vector<bool>& upperDiagonal,vector<bool> &lowerDiagonal)
    {
        if(col==n)
        {
            count++;
        ...
        for(int row=0;row<n;row++)
        {
            if(!LeftRow[row] and  !upperDiagonal[n-1+(col-row)] and !lowerDiagonal[row+col])
            {
                ...
                check(n,board,count,col+1,LeftRow,upperDiagonal,lowerDiagonal);
    ...
    int totalNQueens(int n) 
    {
        ...
        check(n,board,count,col,LeftRow,upperDiagonal,lowerDiagonal);
